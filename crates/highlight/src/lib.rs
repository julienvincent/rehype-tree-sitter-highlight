...
use thiserror::Error;
use tree_sitter::{
  Language, Node, Parser, Point, Query, QueryCapture, QueryCaptures, QueryCursor, QueryError,
  QueryMatch, QueryPredicate, QueryPredicateArg, Range, StreamingIterator, TextProvider, Tree, ffi,
};
...
pub struct HighlightConfiguration {
  ...
  injection_offsets: Vec<HashMap<u32, RangeOffset>>,
...
fn parse_offset_predicate(pred: &QueryPredicate) -> Option<(u32, RangeOffset)> {
  if pred.args.len() != 5 {
    return None;
  }
  ...
  let range = RangeOffset {
    start_row: start_row.as_ref().parse().ok()?,
    start_col: start_col.as_ref().parse().ok()?,
    end_row: end_row.as_ref().parse().ok()?,
    end_col: end_col.as_ref().parse().ok()?,
  };

  Some((*capture, range))
}

fn collect_offset_modifiers(predicates: &[QueryPredicate]) -> HashMap<u32, RangeOffset> {
  let mut map = HashMap::new();
  for pred in predicates {
    if pred.operator.as_ref() != "offset!" {
      continue;
    }
    if let Some((capture, range)) = parse_offset_predicate(pred) {
      map.entry(capture).or_insert(range);
    }
  }
  map
}

fn offset_point(point: Point, row_offset: isize, col_offset: isize) -> Option<Point> {
  let row = point.row as isize + row_offset;
  let col = point.column as isize + col_offset;
  if row < 0 || col < 0 {
    return None;
  }
...
fn byte_to_point(source: &str, byte_offset: usize) -> Option<Point> {
...
fn apply_offset_to_ranges(source: &[u8], ranges: &mut Vec<Range>, offset: &RangeOffset) {
  if ranges.is_empty() {
    return;
  }
  let Ok(source_str) = str::from_utf8(source) else {
    return;
  };

  let mut new_start_point =
    match offset_point(ranges[0].start_point, offset.start_row, offset.start_col) {
      Some(point) => point,
      None => return,
    };
  let mut new_end_point = match offset_point(
    ranges.last().unwrap().end_point,
    offset.end_row,
    offset.end_col,
  ) {
    Some(point) => point,
  ...
  let Some(mut new_start_byte) = point_to_byte(source_str, new_start_point) else {
    return;
  };
  let Some(mut new_end_byte) = point_to_byte(source_str, new_end_point) else {
    return;
  };

  let first_start_byte = ranges[0].start_byte;
  let first_start_point = ranges[0].start_point;
  let last_end_byte = ranges.last().unwrap().end_byte;
  let last_end_point = ranges.last().unwrap().end_point;

  if new_start_byte < first_start_byte {
    new_start_byte = first_start_byte;
    new_start_point = first_start_point;
  }
  if new_end_byte > last_end_byte {
    new_end_byte = last_end_byte;
    new_end_point = last_end_point;
  }
  ...
  if let Some(point) = byte_to_point(source_str, new_start_byte) {
    new_start_point = point;
  }
  if let Some(point) = byte_to_point(source_str, new_end_byte) {
    new_end_point = point;
  }

  while !ranges.is_empty() && new_start_byte >= ranges[0].end_byte {
    ranges.remove(0);
  }
  if ranges.is_empty() {
    return;
  ...
  let first_range = &mut ranges[0];
  if new_start_byte > first_range.start_byte {
    first_range.start_byte = new_start_byte;
    first_range.start_point = new_start_point;
  } else if new_start_byte == first_range.start_byte {
    first_range.start_point = new_start_point;
  }

  while !ranges.is_empty() && new_end_byte <= ranges.last().unwrap().start_byte {
    ranges.pop();
  }
  if ranges.is_empty() {
    return;
  ...
  let last_range = &mut ranges[last_index];
  if new_end_byte < last_range.end_byte {
    last_range.end_byte = new_end_byte;
    last_range.end_point = new_end_point;
  } else if new_end_byte == last_range.end_byte {
    last_range.end_point = new_end_point;
  }

  if let (Some(first), Some(last)) = (ranges.first(), ranges.last()) {
    if first.start_byte >= last.end_byte {
      ranges.clear();
    }
  }
}
...
impl HighlightConfiguration {
  ...
  pub fn new(
    ...
    let injection_offsets = (0..locals_pattern_index)
      .map(|pattern_index| collect_offset_modifiers(query.general_predicates(pattern_index)))
      .collect();
  ...
  #[must_use]
  fn injection_offset(&self, pattern_index: usize, capture_index: u32) -> Option<&RangeOffset> {
    self
      .injection_offsets
      .get(pattern_index)
      .and_then(|offsets| offsets.get(&capture_index))
  }
      ...
      if highlighter.parser.set_included_ranges(&ranges).is_ok() {
        ...
        if let Some(combined_injections_query) = &config.combined_injections_query {
          let mut injections_by_pattern_index =
            vec![(None, Vec::new(), false, None); combined_injections_query.pattern_count()];
          let mut matches = cursor.matches(combined_injections_query, tree.root_node(), source);
          while let Some(mat) = matches.next() {
            let entry = &mut injections_by_pattern_index[mat.pattern_index];
            let (language_name, content_node, include_children, offset) =
              injection_for_match(config, parent_name, combined_injections_query, mat, source);
            if let Some(language_name) = language_name {
              entry.0 = Some(language_name.to_owned());
            }
          ...
          drop(matches);
          for (lang_name, content_nodes, includes_children, offset) in injections_by_pattern_index {
            if let (Some(lang_name), false) = (lang_name.as_deref(), content_nodes.is_empty()) {
              if let Some(next_config) = (injection_callback)(lang_name) {
                let mut injection_ranges =
                  Self::intersect_ranges(&ranges, &content_nodes, includes_children);
                if let Some(offset) = offset {
                  apply_offset_to_ranges(source, &mut injection_ranges, &offset);
                }
                if !injection_ranges.is_empty() {
                  queue.push((next_config, depth + 1, injection_ranges));
                }
              }
            }
          }
        ...
        let captures = unsafe {
          std::mem::transmute::<QueryCaptures<_, _>, _QueryCaptures<_, _>>(cursor_ref.captures(
            &config.query,
            tree_ref.root_node(),
            source,
          ))
  ...
  fn next(&mut self) -> Option<Self::Item> {
    'main: loop {
      ...
      if match_.pattern_index < layer.config.locals_pattern_index {
        let (language_name, content_node, include_children, offset) = injection_for_match(
          layer.config,
        ...
        if let (Some(language_name), Some(content_node)) = (language_name, content_node) {
          if let Some(config) = (self.injection_callback)(language_name) {
            let mut ranges = HighlightIterLayer::intersect_ranges(
              &self.layers[0].ranges,
              &[content_node],
              include_children,
            );
            if let Some(offset) = offset.as_ref() {
              apply_offset_to_ranges(self.source, &mut ranges, offset);
            }
            if !ranges.is_empty() {
        ...
        if Some(capture.index) == layer.config.local_scope_capture_index {
          ...
          for prop in layer.config.query.property_settings(match_.pattern_index) {
            if prop.key.as_ref() == "local.scope-inherits" {
              scope.inherits = prop.value.as_ref().is_none_or(|r| r.as_ref() == "true");
            }
          }
      ...
      if let Some(highlight) = reference_highlight.or(current_highlight) {
        ...
        return self.emit_event(range.start, Some(HighlightEvent::HighlightStart(highlight)));
      }
...
fn injection_for_match<'a>(
  ...
  source: &'a [u8],
) -> (Option<&'a str>, Option<Node<'a>>, bool, Option<RangeOffset>) {
  ...
  for capture in query_match.captures {
    ...
    if index == language_capture_index {
      ...
      offset = config
        .injection_offset(pattern_index, capture.index)
        .cloned();
    }
  }
  ...
  (language_name, content_node, include_children, offset)
}
